    ...
    if (isTouchDevice) {
        ...
        sections.forEach((section) => {
            ...
            section.addEventListener("mouseleave", (event) => {
                ...
                const isExitingVertically =
                    event.clientY < sectionRect.top || event.clientY > sectionRect.bottom;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (isTouchDevice) {
        ...
        sections.forEach((section) => {
            ...
            // Initialisation des compteurs pour chaque section
            clickCounts[section.id] = 0;

            section.addEventListener("click", (event) => {
                event.preventDefault(); // Empêcher le comportement par défaut (navigation)

                // Si une autre section était active, réinitialiser son état
                if (previousSection && previousSection !== section) {
                    const prevBackground = previousSection.querySelector(".hover-background");
                    gsap.to(prevBackground, {
                        left: "-100%",
                    ...
                    clickCounts[previousSection.id] = 0; // Réinitialisation
                ...
                if (clickCounts[section.id] === 1) {
                    ...
                    // Deuxième clic : navigation vers le lien
                    window.location.href = link.getAttribute("href");
                }
            });
        });
        ...
        document.addEventListener("click", (event) => {
            if (!event.target.closest(".index-section") && previousSection) {
                const prevBackground = previousSection.querySelector(".hover-background");
                gsap.to(prevBackground, {
                    left: "-100%",
                    duration: 0.3,
