...
document.addEventListener("DOMContentLoaded", () => {
    async function fetchInstagramData() {
        try {
            // Récupérer les données depuis le serveur (pas besoin de token dans le front)
            const response = await fetch(`http://localhost:3000/fetch-instagram-data`);

            // Vérifiez si la requête est OK
            if (!response.ok) {
            ...
            // Vérifier si une erreur a été retournée par le serveur
            if (data.error) {
                console.error('Erreur API:', data.error);
                alert('Une erreur est survenue lors de la récupération des données Instagram.');
                return;
            }
            ...
            alert('Impossible de récupérer les données Instagram. Veuillez réessayer plus tard.');
        }
    }
    ...
    function displayInstagramData(data) {
        ...
        // Vérifier qu'il y a suffisamment de médias
        if (!media || media.length < 8) {
            console.error('Pas assez de médias pour l\'affichage.');
            alert('Pas assez de médias disponibles.');
            return;
        }

        // Afficher les médias dans leurs blocs respectifs
        media.slice(0, 8).forEach((item, index) => {
            const imageElement = document.getElementById(`img-item-${index}`);
            if (imageElement) {
                imageElement.src = item.media_url;
                imageElement.alt = item.caption || `Instagram image ${index + 1}`;
            }
        });
    }
