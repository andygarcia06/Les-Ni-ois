...
document.addEventListener("DOMContentLoaded", function () {
    const legalMenuItem = document.querySelector('.secondary-menu-a'); // Lien "Infos légales"
    const legalSubmenu = document.querySelector('.legal-submenu'); // Sous-menu associé

    // Vérifiez que les éléments existent dans le DOM
    if (legalMenuItem && legalSubmenu) {
        // Ajout d'un événement de clic sur "Infos légales"
        legalMenuItem.addEventListener("click", function (event) {
            event.preventDefault(); // Empêche le comportement par défaut (redirection)
            legalSubmenu.classList.toggle("visible"); // Ajoute ou enlève la classe 'visible'
        });
    } else {
        console.error("Élément non trouvé : vérifiez vos sélecteurs !");
    }
});
s
