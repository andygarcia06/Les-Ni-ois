...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sections.forEach((section) => {
        ...
        let firstInteractionDone = false; // Indicateur pour la première animation
        ...
        section.addEventListener("mouseenter", (event) => {
            ...
            if (!firstInteractionDone) {
                ...
                if (isEnteringFromLeft) {
                    // Ajouter la classe pour l'entrée depuis la droite
                    background.classList.remove("hover-background-right");
                    background.classList.add("hover-background");
                } else if (isEnteringFromRight) {
                    // Ajouter la classe pour l'entrée depuis la gauche
                    background.classList.remove("hover-background");
                    background.classList.add("hover-background-right");
                }
            }

            // Lancer l'animation d'apparition
            background.style.left = "0";
            firstInteractionDone = true; // Marquer la première interaction comme terminée
        });
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sections.forEach((section) => {
        const background = section.querySelector(".hover-background");
        const content = section.querySelector(".index-content");

        section.addEventListener("mouseenter", () => {
            // Réinitialiser les animations
            content.classList.remove("fade-out");

            // Faire entrer l'arrière-plan blanc
            gsap.to(background, {
                left: 0,
                duration: 0.3,
                ease: "power2.out",
        ...
        section.addEventListener("mouseleave", (event) => {
            const sectionRect = section.getBoundingClientRect();
            const isExitingTop = event.clientY < sectionRect.top;
            const isExitingBottom = event.clientY > sectionRect.bottom;

            // Détecter la sortie vers le haut ou le bas
            if (isExitingTop || isExitingBottom) {
                // Laisser le texte visible
                content.classList.remove("fade-out");
            } else {
                // Animation normale pour la sortie vers les côtés
                gsap.to(background, {
                    left: event.clientX < sectionRect.left ? "-100%" : "100%",
                    duration: 0.3,
                    ease: "power2.in",
                });
                content.classList.add("fade-out"); // Optionnel : effet visuel sur le texte
            }
        });
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    let activeSection = null; // Suivre la section actuellement ouverte
    ...
    // Gérer la sortie de la souris hors de l'écran
    document.addEventListener("mouseleave", (event) => {
        if (activeSection) {
            ...
            const isExitingLeft = event.clientX < 0;
            const isExitingRight = event.clientX > windowWidth;

            if (isExitingTop) {
                gsap.to(background, {
                    top: "-100%", // Fermer vers le haut
                    duration: 0.3,
                    ease: "power2.in",
                });
            } else if (isExitingBottom) {
                gsap.to(background, {
                    top: "100%", // Fermer vers le bas
                    duration: 0.3,
                    ease: "power2.in",
                });
            } else if (isExitingLeft) {
                gsap.to(background, {
                    left: "-100%", // Fermer vers la gauche
                    duration: 0.3,
                    ease: "power2.in",
                });
            } else if (isExitingRight) {
                gsap.to(background, {
                    left: "100%", // Fermer vers la droite
                    duration: 0.3,
                    ease: "power2.in",
                });
            }
