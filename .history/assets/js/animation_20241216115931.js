document.addEventListener("DOMContentLoaded", function () {
    ...
    sections.forEach((section) => {
        ...
        section.addEventListener("mouseleave", (event) => {
            ...
            if (isExitingLeft || isExitingRight || isExitingVertically) {
                ...
                // Masquer le texte uniquement lorsque le hover n'est plus actif
                gsap.to(content, {
                    opacity: 0,
                    duration: 0.5,
                    ease: "power2.in",
    ...
    document.addEventListener("mouseleave", () => {
        if (activeSection) {
            ...
            // Assurez-vous que le texte reste visible tant que hover est actif
            gsap.to(content, {
                opacity: 1,
                duration: 0, // Pas de transition pour que le texte reste visible immédiatement
            });
    ...
    document.addEventListener("mouseleave", (event) => {
        if (activeSection) {
            ...
            // Animation pour cacher uniquement l'arrière-plan
            gsap.to(background, {
                left: "-100%",
                duration: 0.5,
                ease: "power2.in",
            ...
            // Laisser le contenu visible (ne pas changer opacity)
            gsap.to(content, {
    ...
    // Fonction utilitaire pour détecter si la souris sort de la fenêtre
    function isMouseOutOfWindow(event) {
        return event.clientY < 0 || event.clientY > window.innerHeight || event.clientX < 0 || event.clientX > window.innerWidth;
    }
});
