...
document.addEventListener('DOMContentLoaded', function() {
    ...
    openReservationBtns.forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            popupReservation.classList.remove('hide'); // Retirer la classe de fermeture
            popupReservation.classList.add('show'); // Ajouter la classe d'ouverture
        });
    });

    // Fonction pour fermer la popup
    closeReservationBtn.addEventListener('click', function() {
        popupReservation.classList.remove('show'); // Retirer la classe d'ouverture
        popupReservation.classList.add('hide'); // Ajouter la classe de fermeture

        // Retarder la suppression pour correspondre au temps d'animation
        setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function () {
    // Elements for popups
    const popupReservationFrame = document.querySelector('.popup-reservation-frame');
    const popupCarte = document.getElementById('carte-popup');
    const closeCartePopup = popupCarte.querySelector('.close-popup');
    const closeReservationPopup = document.querySelector('.popup-reservation-close');

    // Tabs and content for reservation popup
    const reservationTab = document.getElementById('reservation-tab');
    const privatisationTab = document.getElementById('privatisation-tab');
    const reservationContent = document.getElementById('reservation-content');
    const privatisationContent = document.getElementById('privatisation-content');
    ...
    // Side menu buttons
    const reserverButton = document.querySelector('.submenu li:nth-child(2) a'); // "Réserver"
    const privatiserButton = document.querySelector('.submenu li:nth-child(3) a'); // "Privatiser"
    const carteButton = document.querySelector('.submenu li:nth-child(1) a'); // "La carte"

    // Drag variables
    let isDragging = false;
    let startX, startY, initialX, initialY;

    // Function to show the reservation popup
    function showReservationPopup() {
        popupReservationFrame.style.display = 'block';
        reservationTab.classList.add('active');
        privatisationTab.classList.remove('active');
        reservationContent.style.display = 'block';
        privatisationContent.style.display = 'none';
    ...
    // Function to show the privatisation content in the reservation popup
    function showPrivatisationContent() {
        popupReservationFrame.style.display = 'block';
        privatisationTab.classList.add('active');
        reservationTab.classList.remove('active');
        reservationContent.style.display = 'none';
    ...
    // Function to show the "La carte" popup
    function showCartePopup() {
        popupCarte.style.display = 'block';
    }
    ...
    privatiserButton.addEventListener('click', function (e) {
        e.preventDefault();
        showPrivatisationContent();
    });

    carteButton.addEventListener('click', function (e) {
        e.preventDefault();
        showCartePopup();
    });

    // Close button for "La carte" popup
    closeCartePopup.addEventListener('click', function () {
        popupCarte.style.display = 'none';
    });

    // Close button for reservation popup
    closeReservationPopup.addEventListener('click', function () {
        popupReservationFrame.style.display = 'none';
    });

    // Ensure correct tab functionality in the reservation popup
    reservationTab.addEventListener('click', function () {
        reservationTab.classList.add('active');
        privatisationTab.classList.remove('active');
        reservationContent.style.display = 'block';
    ...
    header.addEventListener('mousedown', function (e) {
        ...
        startX = e.clientX;
        startY = e.clientY;
        initialX = popupReservationFrame.offsetLeft;
        initialY = popupReservationFrame.offsetTop;
    ...
    document.addEventListener('mousemove', function (e) {
        if (isDragging) {
            const dx = e.clientX - startX;
            const dy = e.clientY - startY;
            popupReservationFrame.style.left = initialX + dx + 'px';
            popupReservationFrame.style.top = initialY + dy + 'px';
        }
    });
