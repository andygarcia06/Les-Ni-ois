document.addEventListener("DOMContentLoaded", function () {
    ...
    if (isTouchDevice) {
        // Comportement pour les appareils tactiles ou tablettes
        ...
        sections.forEach((section, index) => {
            ...
            section.addEventListener("click", (event) => {
                ...
                if (previousSection && previousSection !== section) {
                    ...
                    // Réinitialiser le clic pour l'ancienne section
        ...
        sections.forEach((section) => {
            ...
            section.addEventListener("mouseenter", (event) => {
                ...
                const isEnteringFromLeft = event.clientX < sectionRect.left + sectionRect.width / 2;

                // Initialisation explicite de la position de départ
                if (!background.style.left || background.style.left === "100%") {
                    background.style.left = isEnteringFromLeft ? "-100%" : "100%";
                }
            ...
            section.addEventListener("mouseleave", (event) => {
                const sectionRect = section.getBoundingClientRect();
                const isExitingToLeft = event.clientX < sectionRect.left + sectionRect.width / 2;

                // Animation du background en fonction de la sortie
                gsap.to(background, {
                    left: isExitingToLeft ? "-100%" : "100%",
                    duration: 0.5,
                    ease: "power2.in",
                });

                // Réduction de l'opacité du texte
                gsap.to(content, {
                    opacity: 0,
                    duration: 0.5,
                    ease: "power2.in",
        ...
        document.addEventListener("mouseleave", (event) => {
            if (activeSection) {
                ...
                const content = activeSection.querySelector(".index-content");

                gsap.to(background, {
                    left: "100%", // Réinitialisation vers la droite
                    duration: 0.5,
                    ease: "power2.in",
                });

                gsap.to(content, {
                    opacity: 0, // Le texte disparaît après la sortie
                    duration: 0.5,
                    ease: "power2.in",
                });
