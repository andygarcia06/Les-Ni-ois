...
document.addEventListener("DOMContentLoaded", () => {
    const legalLink = document.querySelector(".legal-link"); // Le lien "RESTO"
    const submenuNavLegal = document.querySelector(".legal-submenu-nav");

    // Gérer l'ouverture/fermeture du sous-menu
    legalLink.addEventListener("click", (e) => {
        e.preventDefault(); // Empêche la redirection
        submenuNavLegal.classList.toggle("active"); // Bascule la classe active
    });

    // Gérer la fermeture du sous-menu au clic en dehors
    document.addEventListener("click", (e) => {
        // Vérifie si le clic n'est pas sur le lien ou à l'intérieur du sous-menu
        if (!submenuNav.contains(e.target) && !legalLink.contains(e.target)) {
            submenuNavLegal.classList.remove("active"); // Retire la classe active
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    const legalLink = document.querySelector(".resto-link"); // Utiliser querySelector pour sélectionner un seul élément
    const submenuNavLegal = document.querySelector(".submenu-nav");

    // Gérer l'ouverture du sous-menu
    legalLink.addEventListener("click", (e) => {
        e.preventDefault(); // Empêche la redirection
        submenuNavLegal.classList.add("active"); // Ajoute la classe active
    });

    // Gérer la fermeture au clic à l'extérieur
    document.addEventListener("click", (e) => {
        // Vérifie si le clic est à l'extérieur du sous-menu ou du lien
        if (!submesubmenuNavLegalnuNav.contains(e.target) && e.target !== legalLink) {
            submenuNavLegal.classList.remove("active");
        }
    });
});
